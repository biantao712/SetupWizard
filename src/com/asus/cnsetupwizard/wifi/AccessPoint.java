...
public class AccessPoint extends Preference {
    ...
    /*TT-347151 Save RSSI to prevent the RSSI doesn't refresh after screen rotation*/
    ...
    private NetworkSelectionStatus mNetworkStatus;
    ...
    public AccessPoint(Context context, Bundle savedState) {
        ...
        if (mScanResult != null) {
        	...
        	if(savedState.containsKey(KEY_RSSI)){
        		mRssi = savedState.getInt(KEY_RSSI);
        	}
        	if(savedState.containsKey(KEY_FREQUENCY)){
        		mFrequency = savedState.getInt(KEY_FREQUENCY);
        	}
        }
    ...
    public void saveWifiState(Bundle savedState) {
        ...
        if(mScanResult == null){
        	savedState.putInt(KEY_RSSI, mRssi);
        	savedState.putInt(KEY_FREQUENCY, mFrequency);
        }
    ...
    private void loadConfig(WifiConfiguration config) {
        ...
        mFrequency = 0;
        mConfig = config;
        mNetworkStatus = mConfig.getNetworkSelectionStatus();
    }
...
//                    new int[] {R.attr.wifi_signal}).getDrawable(0));
    ...
    public int compareTo(Preference preference) {
        ...
        if (networkId == WifiConfiguration.INVALID_NETWORK_ID
                && other.networkId != WifiConfiguration.INVALID_NETWORK_ID) return 1;
        if(SystemProperties.get("ro.carrier").equals("KDDI")) {
        	if (mFrequency >= FREQUENCY_5G && other.mFrequency < FREQUENCY_5G) return -1;
        	if (mFrequency < FREQUENCY_5G && other.mFrequency >= FREQUENCY_5G) return 1;
        }
    ...
    private void refresh() {
        ...
        if (mConfig != null && mConfig.status == WifiConfiguration.Status.DISABLED) {
            switch (mNetworkStatus.getNetworkSelectionDisableReason()) {
